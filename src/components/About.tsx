...
const timelineData = [
  ...
  { year: "2025 (Pursuing)", title: "BE (CSE)", company: "St Joseph Engineering College, Mangalore" },
...
const About = () => {
  return (
    ...
    >
        ...
        viewport={{ once: true }}
        className="max-w-5xl mx-auto mb-16 text-xl p-6 bg-white/90 rounded-lg shadow-lg border border-blue-400"
      >
        <h2 className="text-center text-5xl text-blue-400 font-bold mb-3">About Me</h2>
        <p className="text-black-700 text-center leading-relaxed">
      ...
      <h2 className="text-center text-4xl md:text-5xl font-bold text-blue-400 mb-16 relative z-10">
      ...
      <div className="max-w-3xl mx-auto space-y-20">
        {timelineData.map((item, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="relative flex items-center justify-center border border-blue-400 bg-white shadow-2xl rounded-lg p-5"
          >
            <div className="text-center">
              <p className="text-blue-400 text-xl font-semibold">{item.year}</p>
              <h3 className="text-2xl font-bold">{item.title}</h3>
              <p className="text-gray-600 text-xl">{item.company}</p>
            </div>
          </motion.div>
        ))}
      </div>
